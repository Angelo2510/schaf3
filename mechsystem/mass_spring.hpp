...
public:
  ...
  virtual void evaluateDeriv(VectorView<double> x, MatrixView<double> df) const override
  {
    ...
    // Block 1: d(acceleration)/d(positions) from springs (CORRECT)
    for (auto spring : mss.springs())
    {
      auto [c1, c2] = spring.connectors;
      ...
      Vec<D> u = (1.0 / r) * d; // u = (p1-p2)/r
      double k = spring.stiffness;
      double alpha = (r - spring.length) / r; // (r-L)/r

      // K = k * ( u u^T + alpha * (I - u u^T) )
      //   = k * ( alpha*I + (1-alpha)*u u^T )
      for (int a = 0; a < D; ++a)
        for (int b = 0; b < D; ++b)
        {
          double uu = u(a) * u(b);
          double Iab = (a == b) ? 1.0 : 0.0;
          double Kab = k * (alpha * Iab + (1.0 - alpha) * uu);

          // F1 = -k(r-L)u => dF1/dp1 = -K, dF1/dp2 = +K
          if (c1.type == Connector::MASS)
          {
            size_t i1 = c1.nr;
            double m1 = mss.masses()[i1].mass;
            df(i1 * D + a, i1 * D + b) += (-Kab) / m1;

            if (c2.type == Connector::MASS)
            {
              size_t i2 = c2.nr;
              df(i1 * D + a, i2 * D + b) += (+Kab) / m1;
            }
          }

          // F2 = +k(r-L)u => dF2/dp2 = -K, dF2/dp1 = +K
          if (c2.type == Connector::MASS)
          {
            size_t i2 = c2.nr;
            double m2 = mss.masses()[i2].mass;
            df(i2 * D + a, i2 * D + b) += (-Kab) / m2;

            if (c1.type == Connector::MASS)
            {
              size_t i1 = c1.nr;
              df(i2 * D + a, i1 * D + b) += (+Kab) / m2;
            }
          }
        }
    }
